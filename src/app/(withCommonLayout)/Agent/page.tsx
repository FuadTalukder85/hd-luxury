...
const AgentPage = () => {
  ...
  const { data: allAgent, isFetching } = useGetUserQuery({
    role: "Agent",
    page,
    limit: 12,
  });

  const meta = (allAgent as any)?.meta;
    ...
    <div>
      ...
      <Container>
          ...
          {isFetching ? (
            ...
            <div className="md:grid grid-cols-4 gap-5 mt-3 md:mt-10 p-3 md:p-0">
              {allAgent && allAgent.length > 0 ? (
                  ...
                  <div key={agent._id} className="mt-5 md:mt-0 relative">
                      ...
                      <div className="relative group overflow-hidden flex items-center justify-center rounded-xl">
                        {agent?.image ? (
                          <Image
                            ...
                            width={300}
                            height={300}
                            className="rounded-xl hover:scale-105 transition-all duration-700 ease-in-out object-cover"
                          />
                        ) : (
                          <div className="w-[300px] h-[300px] bg-gray-200 rounded-xl flex items-center justify-center text-4xl text-seaBlue font-bold">
                    ...
                    <div className="flex justify-between items-center mt-2 md:p-2">
                      ...
                      <ul className="flex gap-3 text-light text-lg">
                        <li
                          onClick={() =>
                            toggleModal(agent._id, `Call: ${agent?.number || "N/A"}`)
                          }
                ...
                <div className="col-span-4 text-center text-xl text-gray-500 py-10">
                  No agents found
                </div>
              )}
            </div>
        ...
        {meta && meta.totalPages > page && (
          <div className="flex justify-center mt-10" onClick={() => setPage((p) => p + 1)}>
